...
this.paint_topics = function()
	...
	for (var i = 0; i < self.mega_list.length; i++)
		...
		$("#" + name).click(function(){
			...
			if(self.ballsActive === 1)
			{
				
				var id = $(this).attr("topic_id");
				self.hilite_topic(id);
...
this.hilite_topic = function(topic_id)
	{
	self.unhilite_topic();
	
	var s = "#topic_" + topic_id;
	$(s).addClass("active_topic");	
	}


this.unhilite_topic = function()
	{
	for (var i = 0; i < self.topic_list.length; i++)
		{
		if ( $(self.topic_list[i]).hasClass("active_topic") )
			{
			$(self.topic_list[i]).removeClass("active_topic"); 	
			
			var scale = 'scale(1,1)';	
			$(self.topic_list[i]).animate({
  				transform: scale
			}, function(){
			});
...
this.show_documents = function(id)
	...
	if (!x)
		...
		$("#Dokumente").mouseover(function(){
			console.log("Bin über dem Dokument Fenster")
		});	
	...
	for (var i = 0; i < docs.length; i++)
		{
		var str = docs[i].title.replace(/'/g, "\&#39;");	
	
		// Hier klappen die doppelten Anführungszeichen
		// var js 		= 'javascript:tm.show_doc(\'' + docs[i].url + '\',\''   + str+ '\')';
		
		var js 	= 'javascript:tm.new_window_document('  + id + ','  + i +   ')';	
		var js2 	= 'javascript:tm.preview_document('  + id + ','  + i +',this)';		
		var js3 	= 'javascript:tm.hide_span(this)';	
		...
		// alert(js);	
		tx += '<div onmouseout = "' + js3 + '" onmouseover = "' + js2 + '" onclick = "' + js + '" class = "doklink">' + docs[i].title + '<span class = "readmore">MORE >></span></div>';	
...
this.hide_span = function(this_div)
	{
	$(this_div.children[0]).css("color", "transparent");	
	}


this.preview_document = function(topic_id, doc_id, this_div)
	{
	testvar = this_div;
	$(this_div.children[0]).css("color", "blue");	
		
	var url   = self.mega_list[topic_id].docs[doc_id].url;
	var title = self.mega_list[topic_id].docs[doc_id].title;
	$("#DokumentePreviewContainer").html(title);
	}


this.new_window_document = function(topic_id, doc_id)
	{	
	// $("#DocumentView").show();
	var url   = self.mega_list[topic_id].docs[doc_id].url;	
...
this.forward = function()
	...
	timeline.moveBars(0,1,"right");
	//self.setTime();	
	}



this.centerTime = function(time)
	{
	var d = new Date(time);
	var day = d.getDate();
...
this.fast_backward_movement = function()
	{
	self.start_time = self.start_time - 86400000;	
	timeline.moveBars(1,0,"left");
	//self.setTime(); 
...
this.stop_backward_movement = function()
	{	
...
this.fast_forward_movement = function()
	{
	self.start_time = self.start_time + 86400000;
	timeline.moveBars(0,1,"right");	
	//self.setTime(); 
...
this.stop_forward_movement = function()
	{
	window.clearInterval(self.pid);	
	}
...
this.navigation = function()
	...
	$("#fast_backward").mousedown(function(){	
	
		var s = "tm.fast_backward_movement()";
		tm.pid = window.setInterval(s, 50);
	});		
	
	$("#fast_backward").mouseup(function(){	
		tm.stop_backward_movement();	
	});	
	
	$("#fast_forward").mousedown(function(){	
		var s = "tm.fast_forward_movement()";
		tm.pid = window.setInterval(s, 50);
	
	});		
	
	$("#fast_forward").mouseup(function(){	
		tm.stop_forward_movement();	
	});	
...
this.show_search_dialog = function()
	{
	var x = document.getElementById("search_dialog");
	if (!x)
		{
		var s = '<div id = "search_dialog">';
			s += '<div id = "search_dialog_header">SUCHE</div>';
		
			s += '<input type="text" id="lda_search">';
			
		s += '</div>';
		$(self.div).append(s);
		
		
		$("#lda_search").change(function(){
			alert("this feature is not yet implemented");
			$("#search_dialog").fadeOut();
		});
...
this.init_zoom = function()
	...
	config = {};
 	...
 	self.zoom = new Zoomer.Zoom(config);	
	}
...
this.init_pan_listeners = function()
	{
		$('#field_container').mousedown(function(event){
			
			self.testfieldClicked = 1;
			self.mouseXPrev = event.pageX;
			self.mouseYPrev = event.pageY;
			tm.ballsActive = 1;
			//console.log(self.testfieldClicked);
		...
		$('#field_container').mouseup(function(event){
			...
			self.testfieldClicked = 0;
			
			//console.log(self.testfieldClicked);

		});
		
		$('#field_container').mousemove(function(event){
			
			if(self.testfieldClicked === 1)
			{
								
				var moveX = self.mouseXPrev - event.pageX;
				var moveY = self.mouseYPrev - event.pageY;
				
				self.mouseXPrev = event.pageX;
				self.mouseYPrev = event.pageY;
				
				if(moveX > 1 || moveY > 1) tm.ballsActive=0;
				
				var newLeftPosition = $('#testfield').offset().left - moveX;
				var newTopPosition = $('#testfield').offset().top - moveY;
				
				//$('#testfield').position().left
				
				var fieldContainerWidth = $('#field_container').width();
				var fieldContainerHeight = $('#field_container').height();
				var fieldContainerPosLeft = $('#field_container').offset().left;
				var fieldContainerPosTop = $('#field_container').offset().top;
				
				var testfieldWidth = fieldContainerWidth * (testbed.zoom-1);
				var testfieldHeight = fieldContainerHeight * (testbed.zoom-1);
				
				var isValidMove = 1;				
				//console.log(testbed.zoom);
				
				//console.log(fieldContainerWidth);
				//console.log(fieldContainerPosLeft);
				
				/*if(newLeftPosition < fieldContainerPosLeft - testfieldWidth -200)
				{
					isValidMove = 0;
					
				}
				if(newLeftPosition > fieldContainerPosLeft + 200)
				{
					isValidMove = 0;
					//console.log("Dobe vaaaaiii!");
				}
				if(newTopPosition < fieldContainerPosTop - testfieldHeight -200)
				{
					isValidMove = 0;
					//console.log("Dobe vaaaaiii!");
				}
				if(newTopPosition > fieldContainerPosTop + 200)
				{
					isValidMove = 0;
					//console.log("Dobe vaaaaiii!");
				}*/
				...
				if(isValidMove === 1)
				{
					...
					testbed.translationAccX -= moveX;
					testbed.translationAccY -= moveY;
					
					var translate = 'translate('+testbed.translationAccX+'px, '+testbed.translationAccY+'px)';
					var scale = 'scale(' + testbed.zoom + ',' + testbed.zoom + ')';	
					
					
					$('#testfield').css('transform', translate+scale);
					//$('#testfield').offset({ top: newTopPosition, left: newLeftPosition});
				}
...
this.init = function()
	...
	var jason = $.getJSON( "prism.txt", function(data) {
		...
		if (timeline.range.center) tm.centerTime(timeline.range.center);
	})
...
var ow = parseInt($("#testfield").width()) ;
var oh = parseInt($("#testfield").height());
...
// var s = 'translate(' + offx + ',' + offy + ') scale(' + scale + ',' + scale + ')';
var s = 'translate(' + offx + ',' + offy + ')';

console.log(s);
$("#testfield").css('transform', s);	
}
...
function cron_job(date)
{
var url = "http://toskana.ludicmedia.de:7681";	
